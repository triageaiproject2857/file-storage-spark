...
const initialFiles: BackupFile[] = [
  { id: '1', name: 'Q4 Financial Report.xlsx', type: 'spreadsheet', size: '2.4 MB', date: 'Today, 10:42 AM', status: 'Completed' },
  { id: '2', name: 'Project Aurora — Assets.zip', type: 'archive', size: '846 MB', date: 'Today, 9:18 AM', status: 'Completed' },
  { id: '3', name: 'Team offsite photos', type: 'image', size: '124 MB', date: 'Yesterday, 4:36 PM', status: 'Completed' },
  { id: '4', name: 'Product roadmap.docx', type: 'document', size: '1.8 MB', date: 'Yesterday, 2:05 PM', status: 'Completed' },
  { id: '5', name: 'Brand guidelines.pdf', type: 'document', size: '8.2 MB', date: 'Feb 18, 2025', status: 'In progress' },
  { id: '6', name: 'Customer exports.csv', type: 'spreadsheet', size: '42.6 MB', date: 'Feb 17, 2025', status: 'Completed' },
]
...
function Sidebar({ view, setView }: { view: 'backups' | 'admin'; setView: (view: 'backups' | 'admin') => void }) {
...
function UserDashboard() {
  ...
  const handleUpload = async (selected: FileList | File[]) => { const file = selected[0]; if (!file) return; const newFile = await uploadFile(file); setFiles((current) => [newFile, ...current]); toast.success(`${file.name} added to your backups`) }
  ...
  const action = async (kind: 'download' | 'restore' | 'delete', file: BackupFile) => { if (kind === 'delete') { await deleteFile(file); setFiles((current) => current.filter((item) => item.id !== file.id)); toast.success(`${file.name} deleted`) } else { await downloadFile(file); toast.success(kind === 'restore' ? `${file.name} is ready to restore` : `Downloading ${file.name}`) } }
...
export default function CloudVaultDashboard() {
  ...
  return <div className="flex min-h-screen bg-background"><Sidebar view={view} setView={setView} /><div className="flex min-w-0 flex-1 flex-col"><Header view={view} /><main className="flex-1">{view === 'backups' ? <UserDashboard /> : <AdminPanel />}</main><footer className="flex items-center justify-between border-t border-border px-5 py-4 text-[10px] text-muted-foreground md:px-8"><span>CloudVault © 2025</span><span className="flex items-center gap-1.5"><span className="size-1.5 rounded-full bg-emerald-500" />Secure and encrypted</span></footer></div></div>
}
